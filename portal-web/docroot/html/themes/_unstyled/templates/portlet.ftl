...
<section class="portlet" id="portlet_${portlet_id}">
	<header class="portlet-topper">
		<h1 class="portlet-title">
			<#if layout?? && layout.isTypePanel()>
				<#assign portlet_title_css = "portlet-title-text not-editable" />
			<#else>
				<#assign portlet_title_css = "portlet-title-text" />
			</#if>

			${theme.portletIconPortlet()} <span class="${portlet_title_css}">${portlet_title}</span>
		</h1>

...
<liferay-ui:icon-menu showWhenSingleIcon='<%= tabs1.equals("my-sites") %>'>
	<c:choose>
		<c:when test='<%= tabs1.equals("my-sites") %>'>
			...
			<c:if test="<%= ((group.getType() == GroupConstants.TYPE_SITE_OPEN) || (group.getType() == GroupConstants.TYPE_SITE_RESTRICTED)) && GroupLocalServiceUtil.hasUserGroup(user.getUserId(), group.getGroupId(), false) && !SiteMembershipPolicyUtil.isMembershipRequired(user.getUserId(), group.getGroupId()) && !SiteMembershipPolicyUtil.isManualMembershipManagementDisallowed(group.getGroupId()) %>">
		...
		<c:when test="<%= !SiteMembershipPolicyUtil.isManualMembershipManagementDisallowed(group.getGroupId()) %>">
			<c:choose>

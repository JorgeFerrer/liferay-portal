...
public class SecurityPortletContainerWrapper implements PortletContainer {
	...
	public static PortletContainer createSecurityPortletContainerWrapper(
		...
		if (!SPIUtil.isSPI()) {
			portletContainer = new SecurityPortletContainerWrapper(
				portletContainer, portletContainerSecurity);
		}
	...
	protected void check(HttpServletRequest request, Portlet portlet)
		...
		if (!_portletContainerSecurity.isValidPortletId(portlet.getPortletId())) {
		...
		if (layout.isTypeControlPanel()) {
			if (_portletContainerSecurity.isAccessAllowedToControlPanelPortlet(request, portlet)) {
				return;
			}
		...
		if (_portletContainerSecurity.isAccessAllowedToLayoutPortlet(request, portlet)) {
			PortalUtil.addPortletDefaultResource(request, portlet);

			if (_portletContainerSecurity.hasAccessPermission(request, portlet)) {
				return;
			}
		}
	...
	protected void checkAction(HttpServletRequest request, Portlet portlet)
		...
		_portletContainerSecurity.checkCSRFProtection(request, portlet);
	...
	private PortletContainerSecurity _portletContainerSecurity;
}